            ...
            scenes = [
                {
                    ...
                    "elements": [
                        ...
                        {"id": "sub", "type": "TEXT", "text": f"Source: pages {', '.join(map(str, src))}", "size": "small", "color": "grey", "x": 50, "y": 16, "animation": "FADE_IN"},
                    ],
                },
                {
                    ...
                    "clear": False,
                    "elements": [
                        {"id": "flow", "type": "FLOW", "steps": kws[:4], "direction": "horizontal", "x": 50, "y": 34, "w": 84, "h": 12, "color": "blue", "animation": "SEQUENTIAL_REVEAL"},
                    ],
                },
                {
                    ...
                    "elements": [
                        {"id": "d", "type": "DIAGRAM", "animation": "DRAW", "color": "ink",
                         "nodes": [{"id": "n1", "label": kws[0], "x": 25, "y": 62}, {"id": "n2", "label": kws[1 % len(kws)], "x": 50, "y": 78}, {"id": "n3", "label": kws[2 % len(kws)], "x": 75, "y": 62}],
                ...
                },
                {
                    ...
                    "clear": True,
                    "elements": [
                        {"id": "t2", "type": "TEXT", "text": "Key points", "size": "large", "x": 50, "y": 10, "animation": "WRITE"},
                        {"id": "tbl", "type": "TABLE", "rows": [["Idea", "Why it matters"]] + [[k, "see the source pages"] for k in kws[:3]], "x": 50, "y": 45, "w": 76, "h": 40, "animation": "SEQUENTIAL_REVEAL"},
                        {"id": "eq", "type": "EQUATION", "text": f"{name} = {' + '.join(kws[:3])}", "x": 50, "y": 80, "color": "purple", "animation": "WRITE"},
                    ],
                },
            ]
        ...
        "parts": [
            {
                ...
                "scenes": [
                    ...
                    {"narration": "TCP is reliable and ordered, so each side keeps state: sequence numbers, buffers, and window sizes. That state has to be set up before the first byte is sent.", "clear": False,
                     "elements": [
                         {"id": "cs", "type": "TEXT", "text": "seq #, buffers, window", "size": "small", "color": "blue", "x": 20, "y": 62, "animation": "WRITE"},
                         {"id": "ss", "type": "TEXT", "text": "seq #, buffers, window", "size": "small", "color": "green", "x": 80, "y": 62, "animation": "WRITE"},
                         {"id": "q", "type": "TEXT", "text": "How do both sides agree?", "size": "large", "color": "red", "x": 50, "y": 82, "animation": "FADE_IN"},
                     ]},
                    {"narration": "A single request is not enough, because the client would never know whether the server actually received it. So TCP uses an exchange of three messages: the three-way handshake.", "clear": False,
                     "elements": [
                         {"id": "a1", "type": "ARROW", "from": "client", "to": "server", "label": "request?", "color": "grey", "animation": "ARROW_FLOW"},
                ...
                "scenes": [
                    {"narration": f"{slow}Here is the handshake itself. The client starts by sending a SYN segment, which carries its initial sequence number.", "clear": True,
                     "elements": [
                         ...
                         {"id": "server", "type": "BOX", "label": "Server", "x": 82, "y": 30, "w": 18, "h": 11, "color": "green", "animation": "DRAW"},
                         {"id": "syn", "type": "ARROW", "x": 27, "y": 42, "x2": 73, "y2": 50, "label": "1. SYN (seq = x)", "color": "blue", "animation": "ARROW_FLOW"},
                     ]},
                    {"narration": "The server replies with SYN-ACK. It acknowledges the client's sequence number by sending x plus one, and includes its own initial sequence number y.", "clear": False,
                     "elements": [
                         {"id": "synack", "type": "ARROW", "x": 73, "y": 56, "x2": 27, "y2": 64, "label": "2. SYN-ACK (seq = y, ack = x+1)", "color": "green", "animation": "ARROW_FLOW"},
                     ]},
                    {"narration": "Finally the client sends an ACK with y plus one. Now both sides know the other one is alive and has agreed on the starting numbers. The connection is established.", "clear": False,
                     "elements": [
                         {"id": "ack", "type": "ARROW", "x": 27, "y": 70, "x2": 73, "y2": 78, "label": "3. ACK (ack = y+1)", "color": "blue", "animation": "ARROW_FLOW"},
                         {"id": "est", "type": "TEXT", "text": "Connection established", "size": "large", "color": "green", "x": 50, "y": 90, "animation": "WRITE"},
                    ...
                    {"narration": "Remember the pattern: SYN, SYN-ACK, ACK. Three messages, and each side has both sent and received an acknowledgement.", "clear": True,
                     "elements": [
                         ...
                         {"id": "flow", "type": "FLOW", "steps": ["SYN", "SYN-ACK", "ACK", "Established"], "direction": "horizontal", "x": 50, "y": 40, "w": 86, "h": 12, "color": "purple", "animation": "SEQUENTIAL_REVEAL"},
                         {"id": "tbl", "type": "TABLE", "rows": [["Message", "From", "Carries"], ["SYN", "Client", "seq = x"], ["SYN-ACK", "Server", "seq = y, ack = x+1"], ["ACK", "Client", "ack = y+1"]], "x": 50, "y": 72, "w": 76, "h": 32, "animation": "SEQUENTIAL_REVEAL"},
                     ]},
                ],
                "sources": src,
                "ai_added_context": [],
